...
[v39]
- 追加/編集画面の開始日時・終了日時でiOSのdatetime-local固有の最小幅を抑え、右側が枠外へはみ出さないよう再調整。
- カレンダー右側イベント一覧の横スクロールにiOSのネイティブ慣性スクロールを有効化。スワイプの強さに応じて移動量と減速感が変わる操作へ。
- 上段背景を画面切替に関係なく chrome-fill の単色へ固定。
- 開催中ページのイベントカード内枠を minidot-8pink-1〜9 に変更。表示サイズは16px。


[v40]
- 開催中のイベントカード内枠で、中央5番タイルも minidot-8pink-5 を使用するよう修正。枠1〜9すべてpink系を16px表示。
